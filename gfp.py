...
class GFP:
	...
	def generator(self, H, n_pix):
		...
		model.compile(optimizer = tf.keras.optimizers.legacy.Adamax(), loss = 'mse', \
					  metrics = ['mae'])

		NICOLE bug fix: 
		The previous code resulted in
		ValueError: Could not interpret optimizer identifier: <keras.src.optimizers.adamax.Adamax object at 0x2a02a7a10>
		This is because there are two different versions of keras in tensorflow that do not work together (see https://stackoverflow.com/questions/50056356/could-not-interpret-optimizer-identifier-error-in-keras)
		Specifying which keras version avoids this bug

		Right now, tf.keras.optimizers.Adamax runs slowly on M1/M2 Macs, so I have used the legacy Keras optimizer instead (tf.keras.optimizers.legacy.Adamax)
		"""
		x = tf.keras.layers.Input(shape=(2,))
		y = tf.keras.layers.Dense(H,activation='relu',trainable = True)(x)
		y = tf.keras.layers.Dense(H,activation='relu',trainable = True)(y)
		y = tf.keras.layers.Dense(H,activation='relu',trainable = True)(y)
		out = tf.keras.layers.Dense(n_pix,activation='linear',trainable = True)(y)
	...
	def synth_spectrum_sampler(self, wl, teff, logg, rv, specclass = None):
		...
		synth = self.model[specclass].predict(label,verbose=0)[0] #NICOLE bug fix: added verbose=0, otherwise tensorflow Model.predict prints a progress bar with every use
	...
	def fit_spectrum(self, wl, fl, ivar = None, prior_teff = None, mcmc = False, fullspec = False, polyorder = 0, 
		...
		if DA:
			#NICOLE bug fix: SDSS-V data often has a large spectral feature at ~3600 A that will sometimes disrupt the spline fit
			#I built in a feature to crop the spectrum if the initial fit breaks because of this feature
			#wl, fl, ivar = self.spline_norm_DA(wl, fl, ivar, kwargs = norm_kw, crop = crop)
			wl_init, fl_init, ivar_init = self.spline_norm_DA(wl, fl, ivar, kwargs = norm_kw,crop=crop)
			#will get nan values in flux array if spline fit fails
			if np.isnan(fl_init).any()==True:
				print('NaNs detected, cropping the wavelength range of the spectrum to get rid of small wavelength spectral artifacts')
				#find the points where the flux transitions from positive to negative
				transition_indices = []
				for i in range(1, len(fl)):
					if fl[i-1] < 0 and fl[i] >= 0:
						transition_indices.append(i)
				#mask any values>500 datapoints since we care only about the large artifact at low wavelengths
				transition_indices=np.ma.masked_greater(transition_indices, 500)
				#find the last point where this occurs
				crop_val=max(transition_indices)
				crop_wl=int(wl[crop_val]+1)
				wl_init, fl_init, ivar_init = self.spline_norm_DA(wl, fl, ivar, kwargs = norm_kw,crop=(crop_wl,7500))
				extra_crop=0
				while np.isnan(fl_init).any()==True:
					print('Cropping more')
					extra_crop=extra_crop+10
					crop_wl=int(wl[crop_val]+extra_crop)
					wl_init, fl_init, ivar_init = self.spline_norm_DA(wl, fl, ivar, kwargs = norm_kw,crop=(crop_wl,7500))
				wl, fl, ivar=wl_init, fl_init, ivar_init
			else:
				wl, fl, ivar=wl_init, fl_init, ivar_init
